...
import { useChecklistCreate } from "@/services/checklist.service";
import { useDepartmentSelect } from "@/services/department.service";
import { ChecklistCreate, EditableCellProps } from "@/types/checklist";
...
function EditableCell({
  ...
  const save = async () => {
    try {
      ...
      toast.error(`Erro ao salvar - ${errorInfo}`);
    }
  };
...
export function ChecklistCreateForm() {
  const [form] = Form.useForm<ChecklistCreate>();
  const [dataSource, setDataSource] = useState<DataType[]>([]);

  const { mutateAsync: createChecklist, isPending } = useChecklistCreate();
  const { data: teamsItems, isLoading } = useDepartmentSelect();
  ...
  const initialValues: ChecklistCreate = {
    name: "",
    parameters: [],
    department_ids: [],
  };

  const onFinish = (values: ChecklistCreate) => {
    createChecklist({
      name: values.name,
      ...
      .catch(() => toast.error("Erro ao criar lista de tarefas!"));
  };
  return (
    <Card>
      <Form
      ...
      >
        ...
        <Form.Item
          ...
          id="department_ids"
        >
          <Select options={teamsItems} mode="multiple" loading={isLoading} />
        </Form.Item>
