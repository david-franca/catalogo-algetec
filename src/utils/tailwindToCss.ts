...
const convertToCss = (classNames: string[]) => {
  ...
  arbitraryClasses.forEach((className) => {
    try {
      ...
      if (
        arbitrarySupportedClasses[
          property as keyof typeof arbitrarySupportedClasses
        ]
      ) {
        cssCode += `${arbitrarySupportedClasses[property as keyof typeof arbitrarySupportedClasses]}: ${properyValue};\n`;
      }
    } catch (e) {
      console.log(e);
    }
